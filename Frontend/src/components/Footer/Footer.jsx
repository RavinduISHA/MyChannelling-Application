    ...
    <div className="container">
      <div className="flex justify-between flex-col md:flex-row flex-warp gap-[30px]">
        <div>
          ...
          <p className="text-[16px] leading-7 font-[400] text-textColor mt-4">Copyright &#169; {year} developed by Ravindu Umayanga</p>

          <div className="flex items-center gap-3 mt-4">
            {socialLinks.map((link, index)=>(
              <Link to={link.path} key={index} className='w-9 h-9 border border-solid border-[#181A1F] rounded-full flex items-center justify-center group hover:bg-primary hover:border-none'>
                {link.icon}
              </Link>
            ))}
          </div>
